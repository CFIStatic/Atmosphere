#!/usr/bin/env node
/**
 * Apply stripe_event_forget so a failed webhook can be retried.
 *
 * Railway does not run migrations on boot. Without this RPC, stripe_event_seen
 * keeps a failed checkout marked done and Stripe's 500 retry is ignored.
 *
 * Tries, in order:
...
const sqlPath = join(
  here,
  '../supabase/migrations/20260828180000_stripe_event_forget.sql',
);
...
async function applyViaManagementApi(token, ref, label) {
  ...
  console.log(`Applied stripe_event_forget via ${label} Management API.`);
  return true;
}
...
function applyViaPsql(dbUrl, label) {
  ...
  console.log(`Applied stripe_event_forget via ${label}.`);
  return true;
}
...
console.warn(
  'Skipping stripe_event_forget SQL apply — add SUPABASE_ACCESS_TOKEN (or DATABASE_URL / SUPABASE_DB_PASSWORD) to Keys so failed Stripe checkouts can retry.',
);
