#!/usr/bin/env node
/**
 * Persist atmosphere_plan_code + included_fc_seats and make the
 * Field Capture seat allowance read the org's plan (1 / 3 / 10).
 *
...
const sqlPath = join(
  here,
  '../supabase/migrations/20260907220000_atmosphere_self_serve_plans.sql',
);
...
async function applyViaManagementApi(token, ref, label) {
  ...
  console.log(`Applied Atmosphere self-serve plan seats via ${label} Management API.`);
  return true;
}
...
function applyViaPsql(dbUrl, label) {
  ...
  console.log(`Applied Atmosphere self-serve plan seats via ${label}.`);
  return true;
}
...
console.warn(
  'Skipping Atmosphere self-serve plan SQL apply — add SUPABASE_ACCESS_TOKEN (or DATABASE_URL / SUPABASE_DB_PASSWORD).',
);
