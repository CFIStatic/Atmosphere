#!/usr/bin/env node
/**
 * Apply the Start-a-job create repair to the linked Supabase project.
 *
 * Railway does not run migrations on boot. Until this SQL is applied:
 *   - intake_create_job_file still requires at least one invitee
 *   - leftover crm_jobs_audit writes to the dropped crm_audit_log
 * Either one surfaces as "Could not create the job."
 *
 * Tries, in order:
...
async function applyViaManagementApi(token, ref, label) {
  ...
  console.log(`Applied intake job-create repair via ${label} Management API.`);
  return true;
}
...
function applyViaPsql(dbUrl, label) {
  ...
  console.log(`Applied intake job-create repair via ${label}.`);
  return true;
}
...
console.warn(
  'Skipping intake job-create SQL apply — add SUPABASE_ACCESS_TOKEN (or DATABASE_URL / SUPABASE_DB_PASSWORD) to Keys so Start a job can create a file without invitees.',
);
