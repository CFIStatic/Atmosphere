#!/usr/bin/env node
/**
 * Drop leftover old-product tables (CRM/sales/finance/estimator/web-access).
 *
 * Railway does not run migrations on boot. Live Work Verification tables
 * (crm_jobs, Stripe, Field Capture, HomeOwner Report) are not in this SQL.
 *
 * Tries, in order:
...
const sqlPath = join(
  here,
  '../supabase/migrations/20260828220000_drop_old_product_tables.sql',
);
...
async function applyViaManagementApi(token, ref, label) {
  ...
  console.log(`Applied old-product table drop via ${label} Management API.`);
  return true;
}
...
function applyViaPsql(dbUrl, label) {
  ...
  console.log(`Applied old-product table drop via ${label}.`);
  return true;
}
...
console.warn(
  'Skipping old-product table drop — add SUPABASE_ACCESS_TOKEN (or DATABASE_URL / SUPABASE_DB_PASSWORD) to Keys so leftover CRM/sales/finance tables are removed.',
);
