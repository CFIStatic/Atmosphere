#!/usr/bin/env node
/**
 * Apply terms_acceptances table + GRANTs so Platform can record ToS
 * acknowledgments. Production writes with the service_role client.
 *
...
const sqlPath = join(
  here,
  '../supabase/migrations/20260907200000_terms_acceptances_grants.sql',
);
...
async function applyViaManagementApi(token, ref, label) {
  ...
  console.log(`Applied terms_acceptances grants via ${label} Management API.`);
  return true;
}
...
function applyViaPsql(dbUrl, label) {
  ...
  console.log(`Applied terms_acceptances grants via ${label}.`);
  return true;
}
...
console.warn(
  'Skipping terms_acceptances SQL apply — add SUPABASE_ACCESS_TOKEN (or DATABASE_URL / SUPABASE_DB_PASSWORD) so Terms Continue can persist.',
);
