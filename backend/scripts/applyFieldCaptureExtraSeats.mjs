#!/usr/bin/env node
/**
 * Apply org_billing.extra_fc_seats so Work Verification can persist
 * Stripe extra Field Capture seat quantity (allowed = 3 + extra).
 *
 * Railway does not run migrations on boot.
 */
...
const sqlPath = join(
  here,
  '../supabase/migrations/20260907160000_field_capture_extra_seats.sql',
);
...
async function applyViaManagementApi(token, ref, label) {
  ...
  console.log(`Applied Field Capture extra_fc_seats via ${label} Management API.`);
  return true;
}
...
function applyViaPsql(dbUrl, label) {
  ...
  console.log(`Applied Field Capture extra_fc_seats via ${label}.`);
  return true;
}
...
console.warn(
  'Skipping extra_fc_seats SQL apply — add SUPABASE_ACCESS_TOKEN (or DATABASE_URL / SUPABASE_DB_PASSWORD) so Field Capture seat counts persist.',
);
