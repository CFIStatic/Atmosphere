 ...
 * sits on "Writing…" until FFmpeg finishes. Grab one JPEG at the playhead,
 * or a nearby stored still, and write the note against the playhead.
 */
async function settleWatchFrame(opts: {
  ...
  if (frameB64) {
    if (!isVisionConfigured()) {
      ...
      throw new HttpError(
        503,
        'AI analysis needs a model key on the server (ANTHROPIC_API_KEY or GOOGLE_API_KEY).',
