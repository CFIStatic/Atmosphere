...
/** GET /api/portal/projects/:projectId — shares, visibility, recent messages */
staff.get('/projects/:projectId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [shares, visibility, messages] = await Promise.all([
      portalStore.listShares(supabase, projectId),
      portalStore.getVisibility(supabase, projectId, org.orgId),
      portalStore.listProjectMessages(supabase, projectId, 50),
    ]);
...
staff.post('/projects/:projectId/shares', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await portalStore.upsertVisibility(supabase, org.orgId, projectId, userId, {});
...
/** POST /api/portal/projects/:projectId/messages — staff reply */
staff.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Staff inserts go through the user client (RLS allows staff author_kind).
...
guest.get('/report/:token', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { report, visibility } = await loadGuestReport(req.params.token!);
    res.json({
      report,
...
/** GET /api/portal/report/:token/messages */
guest.get('/report/:token/messages', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { admin, share, visibility } = await loadGuestReport(req.params.token!);
    if (!visibility.allowChat) {
      throw forbidden('Chat is not enabled for this report.', 'portal_chat_disabled');
    }
    const messages = await portalStore.listMessages(admin, share.id);
    res.json({ messages });
  } catch (err) {
    next(err);
  }
...
/** POST /api/portal/report/:token/messages */
...
/** POST /api/portal/report/:token/ask — insurance / job Q&A */
guest.post(
  '/report/:token/ask',
  askLimiter,
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!visibility.allowInsuranceQa && !visibility.allowChat) {
      ...
      // Persist the turn so staff can see what homeowners asked.
      if (visibility.allowChat) {
        ...
        await portalStore.insertMessage(admin, {
          ...
          authorName: 'HomeOwner assistant',
          body: reply,
          topic: 'insurance',
        });
      }
