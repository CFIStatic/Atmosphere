...
- `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` (+ `STRIPE_ONBOARDING_PRICE_ID=price_1UDGIZ1b5twUY3LyO0culT5W`, `STRIPE_STARTER_PRICE_ID=price_1UDGIY1b5twUY3Ly7UlLMYBW`, `STRIPE_SCALE_PRICE_ID=price_1UDGIb1b5twUY3LyUuZeyp75`) — see [`docs/stripe.md`](./stripe.md). Annual prices are pinned the same way (env on Railway **Atmosphere APIs** overrides; unset falls back to the live ids): `STRIPE_STARTER_ANNUAL_PRICE_ID=price_1UJL4a1b5twUY3LyozTzumYn`, `STRIPE_ONBOARDING_ANNUAL_PRICE_ID=price_1UJL7O1b5twUY3LysyTksWMm`, `STRIPE_SCALE_ANNUAL_PRICE_ID=price_1UJLA31b5twUY3LysRs2YVmw`, `STRIPE_EXTRA_SEAT_ANNUAL_PRICE_ID=price_1UJLCg1b5twUY3Ly33CATdSo`.
...
That workflow does not assume production is at head. It reconciles the live
schema first and records only the migrations that check out, leaving anything
that looks unapplied for the runner to apply on the next deploy.
...
`supabase/migration-manifest.json` and reports which migrations left no trace.
It only reads.

```bash
npm run migrate:reconcile --prefix backend            # human-readable
node backend/scripts/reconcile-migrations.mjs --json  # machine-readable
node backend/scripts/reconcile-migrations.mjs --baseline-list
```

The manifest is not parsed out of the SQL. Regex over this migration set is not
trustworthy: most files contain dollar-quoted function bodies, and sixteen build
DDL inside `execute format(...)`, so a parser reports objects that are never
created and misses ones that are. Instead the manifest is generated by replaying
every migration against a throwaway Postgres and diffing the schema after each
one, which is exact by construction:

```bash
npm run migrate:manifest --prefix backend -- --url postgresql://postgres@127.0.0.1:5432/postgres
```

Regenerate it whenever migrations are added. `backend/test/migrationReconcile.test.ts`
fails if the manifest and the migration directory disagree.

Three verdicts matter:

- **missing** — none of the objects that migration creates exist. It did not run.
- **partial** — some exist. Either it failed halfway or a later hand-run change
  removed part of it. Re-running is not automatically safe; look at it.
- **unverifiable** — it creates nothing that can be attributed to it. Grants,
  data backfills, comments, objects a later migration dropped, and tables that
  another migration also creates with `if not exists`. The schema cannot show
  whether these ran, and the tool says so rather than guessing.

To baseline around a gap, hand the safe set to the runner and let it apply the
rest:

```bash
node backend/scripts/reconcile-migrations.mjs --baseline-list > safe.txt
node backend/scripts/migrate.mjs --baseline --versions-from safe.txt
node backend/scripts/migrate.mjs                 # applies what was held back
```

### One migration is not in dependency order

`20260807194500_verifier_share_kind.sql` alters `verifier_shares`, which
`20260809090000_verifier_shares.sql` creates. The runner applies in filename
order, so a database cannot be built from these files as they stand. It does
not affect an existing database, where both have long since been applied. The
manifest records it under `outOfOrder`.
