...
assert.equal(Core.PROOF_STILL_COUNT, 3, 'few stills so 30s laptop clips fit under JSON body caps');
assert.equal(Core.PROOF_STILL_MAX_EDGE, 480);
assert.equal(Core.PROOF_FRAMES_JSON_BUDGET, 180000, 'budget stays under historical Express 256kb JSON cap');
assert.match(coreSrc, /PROOF_STILL_MAX_EDGE/, 'proof stills sized for Express JSON body');
assert.match(coreSrc, /extractFrames\(file, PROOF_STILL_COUNT, PROOF_STILL_MAX_EDGE\)/);
