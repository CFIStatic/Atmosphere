/**
 * Wire protocol between the Atmosphere agent (this package) and the Atmosphere
 * backend's agent hub.
 *
 * ⚠️ This file is duplicated verbatim at `backend/src/computer/protocol.ts`.
 * The two packages ship and version independently — an agent installed on a
 * technician's laptop months ago must still talk to today's server — so the
 * protocol is deliberately small, additive-only, and kept in sync by hand.
 * If you change a message here, change it there too.
 */
