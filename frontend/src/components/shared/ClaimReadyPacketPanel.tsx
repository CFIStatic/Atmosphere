 ...
 * Compact card on the job file; full packet only in View (modal).
...
function ClaimReadyPacketDetail({
  packet,
  onRefresh,
  busy,
}: {
  packet: ClaimReadyPacket;
  onRefresh: () => void;
  busy: boolean;
}) {
  return (
    <div className="space-y-4" data-testid="claim-ready-detail">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <button
          ...
          onClick={onRefresh}
          disabled={busy}
          className="shrink-0 rounded-lg border border-line px-2.5 py-1 text-[11px] font-medium text-ink-700 hover:border-brand-400 hover:text-brand-700 disabled:opacity-50"
        >
          ...
          <ul className="mt-1.5 divide-y divide-line overflow-hidden rounded-lg border border-line">
            {packet.parties.map((p) => (
              <li key={p.id} className="px-3 py-2 text-sm text-ink-800">
                ...
                {p.trade ? <span className="ml-1.5 text-xs text-ink-500">{p.trade}</span> : null}
              </li>
            ))}
          </ul>
        ) : (
      ...
      <div>
        ...
        {packet.cause ? (
          <p className="mt-1.5 rounded-lg border border-line/80 px-3 py-2 text-sm text-ink-800">
            {packet.cause.text}
            {packet.cause.quote ? (
              <span className="mt-1 block text-xs italic text-ink-600">“{packet.cause.quote}”</span>
            ) : null}
          </p>
        ) : (
      ...
      <div>
        <h3 className="text-[10.5px] font-semibold uppercase tracking-[0.08em] text-ink-500">Gaps</h3>
  ...
  return (
    <>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-xs text-ink-500">
              Carrier fields from filed evidence only — never invented. Separate from proof-pack PDF.
            </p>
          </div>
      ...
      {open && (
        ...
        >
          <div
            className="flex max-h-[min(90vh,52rem)] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-line bg-paper-0 shadow-lg"
            role="dialog"
            aria-modal="true"
            aria-labelledby={titleId}
            data-testid="claim-ready-modal"
          >
            <div className="flex shrink-0 items-start justify-between gap-3 border-b border-line px-5 py-4">
              <div>
                ...
                <p className="mt-0.5 text-xs text-ink-500">
                  Carrier-ish fields from filed evidence only. Privacy redactions respected.
                </p>
              </div>
              <div className="flex shrink-0 flex-wrap gap-2">
                ...
                <button
                  ...
                  className="rounded-lg border border-line px-2.5 py-1 text-[11px] font-medium text-ink-700 hover:border-brand-400 hover:text-brand-700"
                >
                  Close
                </button>
              </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
              ...
              {busy && !packet && (
                <p className="text-xs text-ink-500">Building claim-ready packet…</p>
              )}
              {packet && (
                <ClaimReadyPacketDetail
                  packet={packet}
                  onRefresh={() => void load()}
                  busy={busy}
                />
