...
describe('parseAnswerSeconds', () => {
  it('reads clocks and spoken Analysis times', () => {
    ...
    expect(parseAnswerSeconds('That was 1 hour and 52 minutes into the recording.')).toEqual([6720]);
  });
});
...
describe('splitAnswerCites', () => {
  it('turns every mentioned moment into a cite at the event-boundary second', () => {
    const parts = splitAnswerCites('At 0:18, the tarp came off. That was 18 seconds into the recording.', [
      8, 18,
    ]);
