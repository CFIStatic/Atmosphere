-- Persist Google-resolved region, country, and coordinates on Approve & invite.
-- The original intake_create_job_file insert only wrote address/city/postal,
-- so UK sites landed as country US with null lat/lng.

drop function if exists public.intake_create_job_file(
...
revoke all on function public.intake_create_job_file(
  uuid, text, text, text, text, text, text, text, double precision, double precision, text, text, jsonb, jsonb, jsonb
) from public;
