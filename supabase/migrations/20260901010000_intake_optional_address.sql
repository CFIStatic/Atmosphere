-- Start a job no longer collects a site address. Allow intake_create_job_file
-- to create the job file without a crm_properties row.
...
comment on function public.intake_create_job_file is
  'Approve & invite: atomically create the job file (optional property, job, '
  'intake, brief, scope, parties) so Job Progress can list it immediately.';
